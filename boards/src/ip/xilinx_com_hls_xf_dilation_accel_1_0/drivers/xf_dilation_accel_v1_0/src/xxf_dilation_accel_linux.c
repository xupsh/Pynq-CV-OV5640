...
typedef struct {
    ...
    XXf_dilation_accel_uio_map maps[ MAX_UIO_MAPS ];
} XXf_dilation_accel_uio_info;

/***************** Variable Definitions **************************************/
static XXf_dilation_accel_uio_info uio_info;
...
static int uio_info_read_name(XXf_dilation_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XXf_dilation_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XXf_dilation_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XXf_dilation_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XXf_dilation_accel_Initialize(XXf_dilation_accel *InstancePtr, const char* InstanceName) {
	XXf_dilation_accel_uio_info *InfoPtr = &uio_info;
